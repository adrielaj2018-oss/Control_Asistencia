    ...
    <div class="phone-wrap desktop-pad tareo-list-page">
      <div class="page-card">
        ...
        <div class="toolstrip tareo-toolbar">
          <a title="Crear hoja" href="{{url_for('crear_hoja')}}"><i class="bi bi-list-task"></i><i class="bi bi-plus" style="font-size:16px;margin-left:-10px"></i></a>
...
def detalle_hoja(hoja_id):
    ...
    <script>
      ...
      function beepOk(){try{beep();}catch(e){} try{const C=window.AudioContext||window.webkitAudioContext;const ctx=new C();const o=ctx.createOscillator();const g=ctx.createGain();o.type='sine';o.frequency.value=880;g.gain.value=.08;o.connect(g);g.connect(ctx.destination);o.start();setTimeout(()=>{o.stop();ctx.close();},120);}catch(e){}}
      function beepBad(){try{const C=window.AudioContext||window.webkitAudioContext;const ctx=new C();const o=ctx.createOscillator();const g=ctx.createGain();o.type='square';o.frequency.value=180;g.gain.value=.06;o.connect(g);g.connect(ctx.destination);o.start();setTimeout(()=>{o.stop();ctx.close();},160);}catch(e){}}
      async function buscarTrabajadorLibre(){let dni=limpiarDni(document.getElementById('buscarDni').value);document.getElementById('buscarDni').value=dni;let box=document.getElementById('buscarResultado');if(dni.length!==8){box.className='alert alert-warning mt-2';box.innerText='Ingrese DNI válido de 8 dígitos.';beepBad();return;}let r=await fetch('/api/trabajador/'+dni);let j=await r.json();if(!j.ok){box.className='alert alert-danger mt-2';box.innerText=j.msg;beepBad();return;}box.className='alert alert-success mt-2';box.innerHTML='<b>'+j.trabajador.trabajador+'</b><br>'+j.trabajador.dni+' · '+(j.trabajador.cargo||'');beepOk();}
      let scanner=null; let colaTrabajadores=[]; let ultimoDni=''; let timerDni=null;
      function setStatus(msg, ok=true){let box=document.getElementById('dniStatus'); if(!box)return; box.className=ok?'mt-2 scan-ok flash':'mt-2 scan-bad'; box.innerHTML=msg;}
      function renderCola(){let q=document.getElementById('workerQueue'), h=document.getElementById('dnisMasivos'); if(!q)return; if(h)h.value=colaTrabajadores.map(x=>x.dni).join(','); if(!colaTrabajadores.length){q.innerHTML='<div class="text-muted small text-center">Aún no hay trabajadores detectados.</div>';return;} q.innerHTML=colaTrabajadores.map(x=>`<div class="queue-item"><div><b>${x.dni}</b><br><span class="text-muted">${x.nombre||'TRABAJADOR DETECTADO'}</span></div><button type="button" class="btn btn-sm btn-outline-danger" onclick="quitarCola('${x.dni}')"><i class="bi bi-x"></i></button></div>`).join('');}
      function quitarCola(dni){colaTrabajadores=colaTrabajadores.filter(x=>x.dni!==dni);renderCola();}
      async function agregarTrabajadorPreRegistro(dni){dni=limpiarDni(dni); if(dni.length!==8)return false; if(colaTrabajadores.some(x=>x.dni===dni)){setStatus('DNI <b>'+dni+'</b> ya está en el pre-registro.', false);beepBad();return false;} try{let r=await fetch('/api/trabajador/'+dni,{cache:'no-store'}); let j=await r.json(); if(!j.ok){setStatus('❌ '+(j.msg||'DNI no encontrado')+' <b>'+dni+'</b>', false);beepBad();return false;} colaTrabajadores.push({dni:dni,nombre:j.trabajador.trabajador||''}); renderCola(); setStatus('✅ Detectado y agregado: <b>'+dni+'</b> · '+(j.trabajador.trabajador||''), true); beepOk(); let inp=document.getElementById('dniTrab'); if(inp){inp.value=''; inp.focus();} return true;}catch(e){setStatus('❌ Error consultando trabajador. Revise conexión o sesión.', false);beepBad();return false;}}
      function autoDetectarDniInline(input){let dni=limpiarDni(input.value); if(dni.length<8)return; if(dni===ultimoDni)return; ultimoDni=dni; clearTimeout(timerDni); timerDni=setTimeout(()=>{agregarTrabajadorPreRegistro(dni).then(()=>{setTimeout(()=>{ultimoDni='';},700);});},120);}
      async function loadQrLib(){if(window.Html5Qrcode)return true; const urls=['https://unpkg.com/html5-qrcode@2.3.8/html5-qrcode.min.js','https://cdn.jsdelivr.net/npm/html5-qrcode@2.3.8/html5-qrcode.min.js']; for(const url of urls){try{await new Promise((res,rej)=>{let s=document.createElement('script');s.src=url;s.onload=res;s.onerror=rej;document.head.appendChild(s);}); if(window.Html5Qrcode)return true;}catch(e){}} return false;}
      async function abrirScanner(readerId,inputId,modo='simple'){let el=document.getElementById(readerId); if(!el)return; el.style.display='block'; el.innerHTML='<div class="p-2 text-success fw-bold">Abriendo cámara...</div>'; if(location.protocol!=='https:' && location.hostname!=='localhost' && location.hostname!=='127.0.0.1'){el.innerHTML='<div class="scan-bad">La cámara requiere HTTPS. Abre la app desde Render con https://</div>'; beepBad(); return;} if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){el.innerHTML='<div class="scan-bad">Este navegador no permite cámara. Usa Chrome actualizado.</div>'; beepBad(); return;} let okLib=await loadQrLib(); if(!okLib){el.innerHTML='<div class="scan-bad">No cargó la librería del lector. Revisa internet/CDN.</div>'; beepBad(); return;} try{if(scanner){await scanner.stop().catch(()=>{}); await scanner.clear().catch(()=>{}); scanner=null;}}catch(e){} try{let formats=[]; if(window.Html5QrcodeSupportedFormats){formats=[Html5QrcodeSupportedFormats.QR_CODE,Html5QrcodeSupportedFormats.CODE_128,Html5QrcodeSupportedFormats.CODE_39,Html5QrcodeSupportedFormats.EAN_13,Html5QrcodeSupportedFormats.EAN_8,Html5QrcodeSupportedFormats.UPC_A,Html5QrcodeSupportedFormats.UPC_E];} scanner=new Html5Qrcode(readerId, formats.length?{formatsToSupport:formats}:{}); await scanner.start({facingMode:{ideal:'environment'}},{fps:12,qrbox:{width:240,height:160},aspectRatio:1.333}, async decoded=>{let dni=limpiarDni(decoded); let inp=document.getElementById(inputId); if(inp)inp.value=dni; if(modo==='cola') await agregarTrabajadorPreRegistro(dni); else beepOk(); try{await scanner.stop(); await scanner.clear();}catch(e){} scanner=null; el.style.display='none';}, ()=>{});}catch(err){el.innerHTML='<div class="scan-bad">No se pudo activar cámara. Permite cámara en el candado del navegador y vuelve a intentar.</div>'; beepBad();}}
      ...
      function bindClock(){document.querySelectorAll('#modalHora input[type=time]').forEach(inp=>{inp.addEventListener('focus',()=>activeTimeInput=inp);inp.addEventListener('click',()=>activeTimeInput=inp);});activeTimeInput=document.getElementById('horaInicioDefault');const face=document.querySelector('#modalHora .clock-face'); if(!face)return;const hand=face.querySelector('.clock-hand'), bubble=face.querySelector('.clock-bubble');const setFromEvent=(ev)=>{const r=face.getBoundingClientRect(); const touch=ev.touches?ev.touches[0]:ev; const cx=r.left+r.width/2, cy=r.top+r.height/2; const x=touch.clientX-cx, y=touch.clientY-cy; let deg=Math.atan2(y,x)*180/Math.PI + 90; if(deg<0)deg+=360; let minutes=Math.round(deg/6/5)*5; if(minutes>=60)minutes=0; const inp=activeTimeInput||document.getElementById('horaInicioDefault'); let [hh]=String(inp.value||'00:00').split(':'); inp.value=String(hh).padStart(2,'0')+':'+String(minutes).padStart(2,'0'); if(hand) hand.style.transform='rotate('+(deg-90)+'deg)'; if(bubble) bubble.textContent=String(minutes); ev.preventDefault();};let drag=false; face.addEventListener('mousedown',e=>{drag=true;setFromEvent(e)}); window.addEventListener('mousemove',e=>{if(drag)setFromEvent(e)}); window.addEventListener('mouseup',()=>drag=false); face.addEventListener('touchstart',e=>{drag=true;setFromEvent(e)},{passive:false}); face.addEventListener('touchmove',e=>{if(drag)setFromEvent(e)},{passive:false}); face.addEventListener('touchend',()=>drag=false);}
      document.addEventListener('DOMContentLoaded',()=>{bindClock(); renderCola(); const inp=document.getElementById('dniTrab'); if(inp){['input','keyup','change','paste'].forEach(ev=>inp.addEventListener(ev,()=>setTimeout(()=>autoDetectarDniInline(inp),20))); inp.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();agregarTrabajadorPreRegistro(inp.value);}});} const frm=document.getElementById('frmTrab'); if(frm){frm.addEventListener('submit',e=>{let inp=document.getElementById('dniTrab');let h=document.getElementById('dnisMasivos'); if(inp && limpiarDni(inp.value).length===8 && !colaTrabajadores.length){e.preventDefault();agregarTrabajadorPreRegistro(inp.value).then(()=>setTimeout(()=>frm.requestSubmit(),180));return;} if(h)h.value=colaTrabajadores.map(x=>x.dni).join(','); if(!h.value && (!inp || limpiarDni(inp.value).length!==8)){e.preventDefault();setStatus('Debe escanear o digitar al menos un DNI válido.', false);}});}});
    </script>
    """
...
def guardar_registro_hoja(hoja_id, tab):
    ...
    dnis = []
    for item in re.split(r'[,;\s]+', dnis_raw):
        dni_limpio = limpiar_dni(item)
        if len(dni_limpio) == 8 and dni_limpio not in dnis:
            dnis.append(dni_limpio)
    if not dnis:
        flash('Debe escanear o digitar al menos un DNI válido de 8 dígitos.', 'danger')
    ...
    metodo = limpiar_texto(request.form.get('metodo') or ('QR/CODIGO/DIGITACION' if len(dnis) > 1 else 'DIGITACION'))

    guardados, no_encontrados, duplicados = 0, [], []
    for dni in dnis:
        t = row_to_dict(execute('SELECT * FROM trabajadores WHERE dni=?', (dni,), fetchone=True))
        if not t:
            no_encontrados.append(dni)
            continue
        ya = scalar('SELECT COUNT(*) AS c FROM tareos WHERE hoja_id=? AND labor_id=? AND dni=?', (hoja_id, labor_id, dni))
        if ya:
        ...
        execute('''INSERT INTO tareos(hoja_id,labor_id,dni,trabajador,empresa,area,cargo,fecha,labor,lote,fundo,horas,cantidad,unidad,observacion,registrado_por,creado_en,hora_inicio,hora_fin,ref_inicio,ref_fin,turno,tipo_tareo)
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                (hoja_id,labor_id,dni,t.get('trabajador',''),t.get('empresa',''),t.get('area',''),t.get('cargo',''),h.get('fecha'),labor,limpiar_texto(request.form.get('lote') or grupo),grupo,horas_reg,cantidad,unidad,'',session.get('usuario'),now_str(),hora_inicio,hora_fin,ref_inicio,ref_fin,turno,tipo_tareo), commit=True)
        guardados += 1

    msg = f'{guardados} trabajador(es) guardado(s) correctamente.'
    if no_encontrados:
        msg += ' No encontrados: ' + ', '.join(no_encontrados) + '.'
    if duplicados:
        msg += ' Ya estaban registrados: ' + ', '.join(duplicados) + '.'
    flash(msg, 'success' if guardados else 'danger')
    return redirect(url_for('detalle_hoja', hoja_id=hoja_id, tab=tab))
