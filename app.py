...
<script>
/* === PATCH 249 FINAL: horario sin duplicar campos, validación avance y auto-guardado === */
(function(){
 const $=id=>document.getElementById(id);
 ...
 let active='horaInicioDefault';
 function syncHidden(){let map=[['horaInicioTrab','horaInicioDefault'],['horaFinTrab','horaFinDefault'],['refInicioTrab','refInicioDefault'],['refFinTrab','refFinDefault']]; map.forEach(([h,i])=>{let he=$(h),ie=$(i); if(he&&ie)he.value=ie.value;});}
 ...
 function bindHorarioFinal(){
   IDS.forEach(id=>{let e=clone(id); if(!e)return; e.readOnly=false; e.inputMode='numeric'; e.maxLength=5; e.addEventListener('focus',()=>setActive(id)); e.addEventListener('pointerdown',()=>setActive(id)); e.addEventListener('click',()=>setActive(id)); e.addEventListener('input',()=>{let v=e.value.replace(/[^0-9:]/g,''); if(/^\d{3,4}$/.test(v)){v=v.padStart(4,'0').replace(/^(\d{2})(\d{2})$/,'$1:$2');} e.value=v.slice(0,5); if(/^([01]?\d|2[0-3]):[0-5]\d$/.test(e.value))repaint();});});
   let pills=$('clockPickFields'); if(pills)pills.querySelectorAll('button').forEach(b=>{let nb=b.cloneNode(true); b.parentNode.replaceChild(nb,b); nb.addEventListener('click',ev=>{ev.preventDefault();ev.stopPropagation();setActive(nb.dataset.target);}); nb.addEventListener('pointerdown',ev=>{ev.preventDefault();ev.stopPropagation();setActive(nb.dataset.target);});});
 ...
 let avanceOK=false;
 async function validarAvanceFinal(){let inp=$('dniAvance'), lab=$('avanceLaborId'), st=$('avanceTrabStatus'), btn=$('btnGuardarAvance'); if(!inp||!lab)return false; let d=onlyDni(inp.value); inp.value=d; avanceOK=false; if(btn)btn.disabled=true; if(d.length<8){if(st){st.className='field-help mt-2';st.innerHTML='Debe estar registrado en Trabajadores de esta labor.';}return false;} try{let m=(($('frmAvance')||{}).action||'').match(/\/hoja\/(\d+)\//); let hoja=m?m[1]:''; let r=await fetch('/api/trabajador-labor/'+hoja+'/'+lab.value+'/'+d,{cache:'no-store',credentials:'same-origin'}); let j=await r.json(); if(j.ok){avanceOK=true; if(btn)btn.disabled=false; if(st){st.className='scan-ok mt-2';st.innerHTML='✓ Trabajador registrado en esta labor: <b>'+j.trabajador.trabajador+'</b>';}; return true;} if(st){st.className='scan-bad mt-2';st.innerHTML='✕ '+(j.msg||'Trabajador no registrado en esta labor.');} return false;}catch(e){if(st){st.className='scan-bad mt-2';st.innerHTML='No se pudo validar trabajador.';}return false;}}
 function parseCantidad(){let c=$('codigoAvance'), q=$('cantidadAvance'), box=$('cantidadDetectada'); if(!c||!q)return; let txt=String(c.value||''); let m=txt.match(/(\d+(?:[\.,]\d+)?)(?!.*\d)/); if(m){q.value=Number(m[1].replace(',','.')).toFixed(2); if(box){box.style.display='block';box.innerHTML='Cantidad a registrar: <b>'+q.value+'</b>';}}}
 let autoTimer=null;
 async function autoGuardarAvance(){clearTimeout(autoTimer); autoTimer=setTimeout(async()=>{parseCantidad(); let ok=await validarAvanceFinal(); let f=$('frmAvance'), cod=$('codigoAvance'); if(ok&&f&&cod&&cod.value.trim()){f.submit();}},350);}
 document.addEventListener('shown.bs.modal',e=>{if(e.target&&e.target.id==='modalHora')setTimeout(bindHorarioFinal,30); if(e.target&&e.target.id==='modalAvance')setTimeout(validarAvanceFinal,80);},true);
 document.addEventListener('input',e=>{if(e.target&&e.target.id==='dniAvance')validarAvanceFinal(); if(e.target&&e.target.id==='codigoAvance')autoGuardarAvance();},true);
 document.addEventListener('submit',e=>{if(e.target&&e.target.id==='frmAvance'&&!avanceOK){e.preventDefault();validarAvanceFinal();alert('El trabajador debe estar registrado en el módulo Trabajadores de esta labor.');}},true);
...
def guardar_registro_hoja(hoja_id, tab):
    ...
    if tab == 'rendimiento':
        ...
        if len(dnis) != 1:
            flash('Registra un trabajador por lectura de avance.', 'danger')
            return redirect(url_for('detalle_hoja', hoja_id=hoja_id, tab=tab, labor_id=labor_id or ''))
        if not scalar('SELECT COUNT(*) AS c FROM tareos WHERE hoja_id=? AND labor_id=? AND dni=?', (hoja_id, labor_id, dnis[0])):
            flash('No se guardó: el trabajador no está registrado en Trabajadores de esta labor.', 'danger')
            return redirect(url_for('detalle_hoja', hoja_id=hoja_id, tab=tab, labor_id=labor_id or ''))
        a_noct = 0.0
