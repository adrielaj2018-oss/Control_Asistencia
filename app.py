    ...
    <div class="phone-wrap desktop-pad"><h2 class="header-title">TAREO MÓVIL – {{ 'DETALLE DE TRABAJADOR POR LABOR' if tab=='trabajadores' else ('DETALLE NÚMERO DE LECTURAS POR BALDE' if tab=='rendimiento' else 'GRUPO DE COSECHA') }}</h2>
      <div class="page-card">
        ...
        {% elif tab=='trabajadores' %}
          {% for r in tareos %}<div class="worker-card"><div class="worker-title"><div>TRABAJADOR<br><b>{{r.trabajador}}</b></div><div>NRO.DOCUMENTO<br><b>{{r.dni}}</b></div></div><div class="worker-grid"><div><label>HORA INICIO</label><input class="mini-input locked-input" readonly onclick="abrirEditarTareo(this)" value="{{r.hora_inicio or ('22:00' if r.turno=='NOCHE' else '06:30')}}"></div><div><label>HORA FIN</label><input class="mini-input locked-input" readonly onclick="abrirEditarTareo(this)" value="{{r.hora_fin or ('06:00' if r.turno=='NOCHE' else '16:30')}}"></div><div><label>H.NORMAL</label><input class="mini-input locked-input" readonly onclick="abrirEditarTareo(this)" value="{{'%.2f'|format((r.horas or 0) - (r.horas_nocturnas or 0))}}"></div><div><label>H.NOCTURNO</label><input class="mini-input locked-input" readonly onclick="abrirEditarTareo(this)" value="{{'%.2f'|format(r.horas_nocturnas or 0)}}"></div><div><label>REF. INI</label><input class="mini-input locked-input" readonly onclick="abrirEditarTareo(this)" value="{{r.ref_inicio or '12:00'}}"></div><div><label>REF. FIN</label><input class="mini-input locked-input" readonly onclick="abrirEditarTareo(this)" value="{{r.ref_fin or '13:00'}}"></div><div><label>ESTADO</label><div class="mini-badge bg-g">FIN TOTAL</div></div></div></div>{% else %}<div class="worker-card text-center text-muted">Presiona el <b>hombresito +</b> para registrar trabajador por QR/código/digitación.</div>{% endfor %}
        {% else %}
