...
<style>
/* PATCH FINAL 250: avance cantidad, cámara con cerrar, horario sin cruce de campos */
#modalAvance .avance-cam-wrap{position:relative;margin-top:8px}#modalAvance .stop-scan-btn{position:absolute;right:6px;top:6px;z-index:9999;border:0;border-radius:999px;background:#dc2626;color:#fff;width:30px;height:30px;font-weight:900;box-shadow:0 2px 8px rgba(0,0,0,.25)}
#avanceQueue .queue-item b{color:#166534}#avanceQueue .queue-item .qty{font-weight:900;color:#166534;font-size:13px}.avance-grid label{font-size:8px!important}.avance-grid .mini-badge{justify-content:center!important;text-align:center!important}
</style>
<script>
(function(){
'use strict';
...
function minToTime(m){m=Math.max(0,Math.min(1435,Math.round(Number(m||0)/5)*5));return pad(Math.floor(m/60))+':'+pad(m%60);}
function ids(){return ['horaInicioDefault','horaFinDefault','refInicioDefault','refFinDefault'];}
let active='horaInicioDefault';
function setActive(id){if(!ids().includes(id))return; active=id; const sl=$('timeSlider24'), tv=$('touchClockValue'), e=$(active); if(e){if(sl)sl.value=toMin(e.value); if(tv)tv.textContent=e.value; try{e.focus({preventScroll:true});}catch(_){}} const box=$('clockPickFields'); if(box)box.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.target===active));}
function syncTrab(){const hi=$('horaInicioDefault')?.value||'',hf=$('horaFinDefault')?.value||'',ri=$('refInicioDefault')?.value||'',rf=$('refFinDefault')?.value||''; [['horaInicioTrab',hi],['horaFinTrab',hf],['refInicioTrab',ri],['refFinTrab',rf]].forEach(([id,v])=>{const e=$(id);if(e)e.value=v;});}
function applyTime(v){const e=$(active); if(!e)return; e.value=minToTime(v); const tv=$('touchClockValue'); if(tv)tv.textContent=e.value; syncTrab();}
function bindHorarioFinal(){const box=$('clockPickFields'); if(box){box.querySelectorAll('button').forEach(b=>{['pointerdown','mousedown','click','touchstart'].forEach(ev=>b.addEventListener(ev,function(e){e.preventDefault();e.stopImmediatePropagation();setActive(b.dataset.target);},true));});}
ids().forEach(id=>{const e=$(id); if(!e || e.dataset.finalHorario==='1')return; e.dataset.finalHorario='1'; e.readOnly=false; ['focus','click','pointerdown','touchstart'].forEach(ev=>e.addEventListener(ev,function(x){x.stopImmediatePropagation();setActive(id);},true)); e.addEventListener('input',()=>{e.value=e.value.replace(/[^0-9:]/g,'').slice(0,5); const sl=$('timeSlider24'),tv=$('touchClockValue'); if(sl)sl.value=toMin(e.value); if(tv)tv.textContent=e.value; syncTrab();},true);});
const sl=$('timeSlider24'); if(sl && sl.dataset.finalHorario!=='1'){sl.dataset.finalHorario='1'; const h=e=>{e.preventDefault();e.stopImmediatePropagation();applyTime(sl.value);}; ['input','change','click','pointerdown','pointermove','touchstart','touchmove','mousemove'].forEach(ev=>sl.addEventListener(ev,function(e){if((ev==='pointermove'||ev==='mousemove') && !(e.buttons||e.touches))return; h(e);},true));}
setActive(active); syncTrab();}

let qrScanner=null;
window.cerrarScanner=function(readerId){const el=$(readerId); try{if(qrScanner){qrScanner.stop().catch(()=>{});qrScanner.clear().catch(()=>{});qrScanner=null;}}catch(e){} if(el){el.style.display='none';el.innerHTML='';}};
async function loadQr(){if(window.Html5Qrcode)return true; return new Promise(res=>{const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/html5-qrcode@2.3.8/html5-qrcode.min.js';s.onload=()=>res(!!window.Html5Qrcode);s.onerror=()=>res(false);document.head.appendChild(s);});}
window.abrirScanner=async function(readerId,inputId){const el=$(readerId), inp=$(inputId); if(!el||!inp)return; el.classList.add('avance-cam-wrap'); el.style.display='block'; el.innerHTML='<button type="button" class="stop-scan-btn" onclick="cerrarScanner(\''+readerId+'\')">×</button><div class="p-2 text-success fw-bold">Abriendo cámara...</div>'; if(location.protocol!=='https:'&&location.hostname!=='localhost'&&location.hostname!=='127.0.0.1'){el.innerHTML='<button type="button" class="stop-scan-btn" onclick="cerrarScanner(\''+readerId+'\')">×</button><div class="scan-bad">La cámara requiere HTTPS.</div>';return;} if(!(await loadQr())){el.innerHTML='<button type="button" class="stop-scan-btn" onclick="cerrarScanner(\''+readerId+'\')">×</button><div class="scan-bad">No cargó librería del lector.</div>';return;} try{if(qrScanner){await qrScanner.stop().catch(()=>{}); await qrScanner.clear().catch(()=>{});}}catch(e){} try{qrScanner=new Html5Qrcode(readerId); await qrScanner.start({facingMode:{ideal:'environment'}},{fps:12,qrbox:{width:240,height:160}},async decoded=>{let val=decoded; if(inputId==='dniTrab'||inputId==='dniAvance')val=onlyDni(decoded); inp.value=val; inp.dispatchEvent(new Event('input',{bubbles:true})); if(inputId==='dniTrab'&&window.autoDetectarDniInline)window.autoDetectarDniInline(inp); if(inputId==='dniAvance')setTimeout(()=>{$('codigoAvance')&&$('codigoAvance').focus();},120); if(inputId==='codigoAvance')setTimeout(()=>addAvancePre(),150); window.cerrarScanner(readerId);},()=>{});}catch(e){el.innerHTML='<button type="button" class="stop-scan-btn" onclick="cerrarScanner(\''+readerId+'\')">×</button><div class="scan-bad">No se pudo activar cámara. Permite cámara en el navegador.</div>';}};

const avanceMap=new Map(); let dniOk=false, nombreOk='';
function setAvanceStatus(ok,msg){const e=$('avanceTrabStatus'); if(!e)return; e.className=ok?'scan-ok mt-2':'scan-bad mt-2'; e.innerHTML=msg;}
function renderAvanceQueue(){const q=$('avanceQueue'), h=$('avancesPayload'); if(!q||!h)return; const arr=[...avanceMap.values()]; h.value=JSON.stringify(arr); if(!arr.length){q.innerHTML='<div class="text-muted small text-center">Lee/digita DNI y cantidad. Se acumula aquí hasta GUARDAR AVANCE.</div>';return;} q.innerHTML=arr.map(x=>'<div class="queue-item"><div><b>'+x.dni+'</b><br><span>'+x.nombre+'</span></div><div class="qty">'+Number(x.cantidad||0).toFixed(2)+'</div><button type="button" class="btn btn-sm btn-outline-danger" onclick="window.avanceMapFinal.delete(\''+x.dni+'\');window.renderAvanceQueueFinal();">×</button></div>').join('');}
window.avanceMapFinal=avanceMap; window.renderAvanceQueueFinal=renderAvanceQueue;
async function validarDniAvance(){const inp=$('dniAvance'), labor=$('avanceLaborId'); if(!inp||!labor)return false; const dni=onlyDni(inp.value); inp.value=dni; dniOk=false; nombreOk=''; if(dni.length!==8){setAvanceStatus(false,'Digite o escanee DNI de 8 dígitos.');return false;} const m=String($('frmAvance')?.action||'').match(/\/hoja\/(\d+)\//); const hoja=m?m[1]:''; try{const r=await fetch('/api/trabajador-labor/'+hoja+'/'+labor.value+'/'+dni,{cache:'no-store',credentials:'same-origin'}); const j=await r.json(); if(j.ok){dniOk=true; nombreOk=(j.trabajador&&j.trabajador.trabajador)||'TRABAJADOR'; setAvanceStatus(true,'✓ Trabajador registrado en esta labor: <b>'+nombreOk+'</b>'); setTimeout(()=>{$('codigoAvance')&&$('codigoAvance').focus();},60); return true;} setAvanceStatus(false,'✕ '+(j.msg||'El trabajador no está registrado en Trabajadores de esta labor.')); return false;}catch(e){setAvanceStatus(false,'No se pudo validar trabajador.');return false;}}
function detectCantidad(){const c=$('codigoAvance'), q=$('cantidadAvance'), box=$('cantidadDetectada'); if(!q)return 0; let txt=String((c&&c.value)||q.value||''); let m=txt.match(/(\d+(?:[\.,]\d+)?)(?!.*\d)/); let val=m?Number(m[1].replace(',','.')):Number(q.value||0); if(!isFinite(val)||val<=0)val=0; if(val>0){q.value=val.toFixed(2); if(box){box.style.display='block';box.innerHTML='Cantidad detectada: <b>'+q.value+'</b>';}} return val;}
async function addAvancePre(){if(!(await validarDniAvance()))return false; const dni=onlyDni($('dniAvance').value), cant=detectCantidad(); if(!cant){alert('Digite o lea una cantidad válida.');return false;} avanceMap.set(dni,{dni,nombre:nombreOk||dni,cantidad:cant}); renderAvanceQueue(); if($('codigoAvance'))$('codigoAvance').value=''; if($('dniAvance')){$('dniAvance').value=''; $('dniAvance').focus();} setAvanceStatus(true,'✓ Agregado al pre-registro: <b>'+dni+'</b>'); return true;}
window.addAvancePreFinal=addAvancePre;
function bindAvanceFinal(){const d=$('dniAvance'), c=$('codigoAvance'), b=$('btnAgregarAvancePre'), f=$('frmAvance'); if(d&&!d.dataset.finalAvance){d.dataset.finalAvance='1'; d.addEventListener('input',()=>{if(onlyDni(d.value).length===8)validarDniAvance();},true); d.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();validarDniAvance();}},true);} if(c&&!c.dataset.finalAvance){c.dataset.finalAvance='1'; let t=null; c.addEventListener('input',()=>{detectCantidad(); clearTimeout(t); t=setTimeout(()=>{if(c.value.trim())addAvancePre();},650);},true); c.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key==='Tab'){e.preventDefault();addAvancePre();}},true);} if(b&&!b.dataset.finalAvance){b.dataset.finalAvance='1'; b.addEventListener('click',addAvancePre,true);} if(f&&!f.dataset.finalAvance){f.dataset.finalAvance='1'; f.addEventListener('submit',async e=>{if(avanceMap.size===0){e.preventDefault(); const ok=await addAvancePre(); if(ok)f.submit();}},true);} renderAvanceQueue();}
window.abrirEditarAvance=function(id,cant,noct){const f=$('frmEditAvance'); if(!f){alert('No se pudo abrir editor.');return;} f.action='/lectura/'+id+'/editar'; $('editCantAvance').value=Number(cant||0).toFixed(2); if($('editNoctAvance'))$('editNoctAvance').value='0.00'; new bootstrap.Modal($('modalEditAvance')).show();};
document.addEventListener('shown.bs.modal',e=>{if(e.target&&e.target.id==='modalHora')setTimeout(bindHorarioFinal,80); if(e.target&&e.target.id==='modalAvance')setTimeout(bindAvanceFinal,80);},true);
document.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{bindHorarioFinal();bindAvanceFinal();},150);},true);
})();
</script>
...
def guardar_registro_hoja(hoja_id, tab):
    ...
    cantidades_por_dni = {}
    if tab == 'rendimiento':
        payload_raw = request.form.get('avances_payload') or ''
        if payload_raw:
            try:
                payload = json.loads(payload_raw)
                dnis = []
                for item in payload:
                    d = limpiar_dni(item.get('dni'))
                    if len(d) == 8 and d not in dnis:
                        dnis.append(d)
                    try:
                        cantidades_por_dni[d] = float(item.get('cantidad') or 0)
                    except Exception:
                        cantidades_por_dni[d] = 0
            except Exception:
                pass
    if not dnis:
    ...
    for dni in dnis:
        ...
        h_reg = horas
        if tab == 'rendimiento':
            cantidad_reg = cantidades_por_dni.get(dni, cantidad)
            execute('INSERT INTO lecturas_balde(hoja_id,labor_id,dni,trabajador,fecha_hora,a_diurno,a_noct,metodo,registrado_por) VALUES(?,?,?,?,?,?,?,?,?)',
                    (hoja_id,labor_id,dni,t.get('trabajador',''),now_str(),cantidad_reg,0,metodo,session.get('usuario')), commit=True)
            ok += 1
            continue
